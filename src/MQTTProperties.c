  ...
  if (props->array)
  {
    ...
    switch (type)
    {
      ...
      case MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR:
        len = 2 + prop->value.propertyDataValue.data.len;
        props->array[props->count-1].value.propertyDataValue.data.data = datadup(&prop->value.propertyDataValue.data);
        if (type == MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR)
        {
          len += 2 + prop->value.propertyDataValue.value.len;
          props->array[props->count-1].value.propertyDataValue.value.data = datadup(&prop->value.propertyDataValue.value);
        }
        break;
    }
...
int MQTTProperty_write(char** pptr, MQTTProperty* prop)
{
  ...
  if (type >= MQTTPROPERTY_TYPE_BYTE && type <= MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR)
  {
    ...
    switch (type)
    {
      ...
      case MQTTPROPERTY_TYPE_BINARY_DATA:
      case MQTTPROPERTY_TYPE_UTF_8_ENCODED_STRING:
        writeMQTTLenString(pptr, prop->value.propertyDataValue.data);
        rc = prop->value.propertyDataValue.data.len + 2; /* include length field */
        break;
      case MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR:
        writeMQTTLenString(pptr, prop->value.propertyDataValue.data);
        writeMQTTLenString(pptr, prop->value.propertyDataValue.value);
        rc = prop->value.propertyDataValue.data.len + prop->value.propertyDataValue.value.len + 4; /* include length fields */
        break;
    }
  }
...
int MQTTProperty_read(MQTTProperty* prop, char** pptr, char* enddata)
{
  ...
  if (type >= MQTTPROPERTY_TYPE_BYTE && type <= MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR)
  {
    switch (type)
    {
      ...
      case MQTTPROPERTY_TYPE_UTF_8_ENCODED_STRING:
      case MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR:
        if ((len = MQTTLenStringRead(&prop->value.propertyDataValue.data, pptr, enddata)) == -1)
          break; /* error */
        if ((prop->value.propertyDataValue.data.data = datadup(&prop->value.propertyDataValue.data)) == NULL)
        {
          len = -1;
          break; /* error */
        }
        if (type == MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR)
        {
          int proplen = MQTTLenStringRead(&prop->value.propertyDataValue.value, pptr, enddata);

          if (proplen == -1)
          {
            len = -1;
            free(prop->value.propertyDataValue.data.data);
            break;
          }
          len += proplen;
          if ((prop->value.propertyDataValue.value.data = datadup(&prop->value.propertyDataValue.value)) == NULL)
          {
            len = -1;
            free(prop->value.propertyDataValue.data.data);
            break;
...
void MQTTProperties_free(MQTTProperties* props)
{
  ...
  for (i = 0; i < props->count; ++i)
  {
    ...
    case MQTTPROPERTY_TYPE_BINARY_DATA:
    	case MQTTPROPERTY_TYPE_UTF_8_ENCODED_STRING:
    	case MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR:
    	  free(props->array[i].value.propertyDataValue.data.data);
    	  if (type == MQTTPROPERTY_TYPE_UTF_8_STRING_PAIR)
    	    free(props->array[i].value.propertyDataValue.value.data);
	  break;
    }
  }
  if (props->array)
